...
export default function ClubDetail() {
  ...
  const [club, setClub] = useState<MMAClub | null>(null);
  ...
  useEffect(() => {
    async function fetchClub() {
      ...
      try {
        ...
        console.error("Error loading club details from Supabase:", err);
      } finally {
        setLoading(false);
      }
    }
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#030303] text-white flex flex-col items-center justify-center gap-3">
        ...
        <span className="text-xs font-mono text-zinc-500 uppercase tracking-widest">Đang tải hồ sơ võ đường...</span>
      </div>
    );
  }
  ...
  if (!club) {
    return (
      <div className="min-h-screen bg-[#030303] text-white flex flex-col items-center justify-center gap-4">
        <span className="text-sm text-zinc-500 font-mono">Không tìm thấy võ đường yêu cầu</span>
        <button onClick={() => navigate("/clubs")} className="px-5 py-2.5 rounded-xl border border-zinc-800 bg-zinc-900/60 text-xs font-mono">
          QUAY LẠI
        </button>
      </div>
    );
