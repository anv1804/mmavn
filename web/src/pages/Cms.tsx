...
export default function Cms() {
  ...
  return (
    <div className={`min-h-screen pt-32 pb-24 relative ${isDark ? "bg-[#030303]" : "bg-[#f9f9fc]"}`} style={{ fontFamily: "'Outfit', sans-serif" }}>
      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        {/* Page title */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b pb-6 mb-8" style={{ borderColor: isDark ? "#1f1f23" : "#e4e4e7" }}>
          <div>
            <span className="text-[10px] font-mono tracking-[0.25em] text-red-500 uppercase font-black">HỆ THỐNG QUẢN TRỊ NỘI DUNG</span>
            <h1 className={`text-4xl font-black uppercase mt-1 ${isDark ? "text-white" : "text-zinc-900"}`} style={{ fontFamily: "'Oswald', sans-serif" }}>
              MMAVN CONTENT MANAGER (CMS)
            </h1>
          </div>
          
          <div className="flex items-center gap-3">
            <span className={`text-[10px] font-mono border rounded px-2.5 py-1 ${isDark ? "border-zinc-800 text-zinc-500" : "border-zinc-200 text-zinc-600"}`}>
              DATABASE STATUS: {loading ? "CONNECTING..." : "ONLINE"}
            </span>
          </div>
        </div>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 space-y-4">
            ...
            <p className="text-xs font-mono text-zinc-500 uppercase tracking-widest">Đang kết nối cơ sở dữ liệu Supabase...</p>
          </div>
        ) : (
          <>
        ...
        {/* Tab Selection */}
        <div className="flex border-b mb-6" style={{ borderColor: isDark ? "#1f1f23" : "#e4e4e7" }}>
          {(["clubs", "fighters", "rankings", "events"] as const).map((tab) => (
            <button
              key={tab}
              onClick={() => { setActiveTab(tab); setSearchQuery(""); }}
              className={`px-6 py-3 text-xs font-mono font-bold uppercase border-b-2 transition-all cursor-pointer border-none bg-transparent ${
                activeTab === tab
                  ? "border-red-500 text-red-500"
                  : "border-transparent text-zinc-500 hover:text-zinc-300"
              }`}
            >
              {tab === "clubs" ? "🏢 VÕ ĐƯỜNG" : tab === "fighters" ? "🥋 VÕ SĨ" : tab === "rankings" ? "🏆 HẠNG CÂN / BXH" : "📅 LỊCH SỰ KIỆN"}
            </button>
          ))}
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {filteredClubs.map((club) => (
              <div key={club.id} className={`p-4 border rounded-2xl flex items-center justify-between transition-all ${
                isDark ? "bg-zinc-900/30 border-zinc-900/80 hover:border-zinc-800" : "bg-white border-zinc-200"
              }`}>
                <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => setSelectedClub(club)} className="px-3 py-1.5 bg-zinc-850 hover:bg-zinc-800 border-none rounded-lg text-[10px] text-zinc-400 font-bold uppercase tracking-wider cursor-pointer">Sửa</button>
                  <button onClick={() => deleteClub(club.id)} className="px-3 py-1.5 bg-red-950/20 hover:bg-red-600 text-red-500 hover:text-white border-none rounded-lg text-[10px] font-bold uppercase tracking-wider cursor-pointer">Xóa</button>
                </div>
              </div>
            ))}
            {filteredClubs.length === 0 && <p className="text-zinc-500 text-xs py-4 font-mono">Không có dữ liệu phù hợp.</p>}
          </div>
        )}

        {/* ── TAB 2: FIGHTERS MANAGEMENT ── */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {filteredFighters.map((fighter) => (
              <div key={fighter.id} className={`p-4 border rounded-2xl flex items-center justify-between transition-all ${
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => setSelectedFighter(fighter)} className="px-3 py-1.5 bg-zinc-850 hover:bg-zinc-800 border-none rounded-lg text-[10px] text-zinc-400 font-bold uppercase tracking-wider cursor-pointer">Sửa</button>
                  <button onClick={() => deleteFighter(fighter.id)} className="px-3 py-1.5 bg-red-950/20 hover:bg-red-600 text-red-500 hover:text-white border-none rounded-lg text-[10px] font-bold uppercase tracking-wider cursor-pointer">Xóa</button>
                </div>
              </div>
            ))}
            {filteredFighters.length === 0 && <p className="text-zinc-500 text-xs py-4 font-mono">Không có dữ liệu phù hợp.</p>}
          </div>
        )}

        {/* ── TAB 3: RANKINGS MANAGEMENT ── */}
            ...
            {filteredRankings.map((ranking) => (
              <div key={ranking.id} className={`p-4 border rounded-2xl flex items-center justify-between transition-all ${
                ...
                <div>
                  ...
                  <p className="text-[10px] text-zinc-500 font-mono">Đương kim vô địch: {ranking.champion?.name || "Bỏ trống"}</p>
                </div>
                <button onClick={() => setSelectedRanking(ranking)} className="px-3 py-1.5 bg-zinc-850 hover:bg-zinc-800 border-none rounded-lg text-[10px] text-zinc-400 font-bold uppercase tracking-wider cursor-pointer">Quản lý BXH</button>
              </div>
            ))}
          </div>
        )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {filteredEvents.map((evt) => (
              <div key={evt.id} className={`p-4 border rounded-2xl flex items-center justify-between transition-all ${
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => setSelectedEvent(evt)} className="px-3 py-1.5 bg-zinc-850 hover:bg-zinc-800 border-none rounded-lg text-[10px] text-zinc-400 font-bold uppercase tracking-wider cursor-pointer">Sửa</button>
                  <button onClick={() => deleteEvent(evt.id)} className="px-3 py-1.5 bg-red-950/20 hover:bg-red-600 text-red-500 hover:text-white border-none rounded-lg text-[10px] font-bold uppercase tracking-wider cursor-pointer">Xóa</button>
                </div>
              </div>
            ))}
            {filteredEvents.length === 0 && <p className="text-zinc-500 text-xs py-4 font-mono">Không có dữ liệu phù hợp.</p>}
          </div>
        )}
