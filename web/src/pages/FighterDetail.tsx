      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 pt-8 pb-20">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-8 space-y-8">
            ...
            {/* ── REDESIGNED FIGHT HISTORY ── */}
            <div className="rounded-2xl border border-zinc-900 bg-zinc-950/40 p-6 space-y-6">
              <div className="flex items-center justify-between border-b border-zinc-900 pb-4">
                <span className="text-[10px] font-mono tracking-[0.2em] text-zinc-500 uppercase">LỊCH SỬ THI ĐẤU (BATTLE CHRONICLE)</span>
              ...
              {/* Grid-based align table (No floating center text) */}
              <div className="space-y-3">
                {fighter.fights?.map((fight, idx) => {
                  ...
                  return (
                    <div 
                      key={idx} 
                      className="group bg-black/40 border border-zinc-900/60 hover:border-red-500/20 hover:bg-zinc-900/10 rounded-xl p-4 transition-all duration-200 grid grid-cols-12 items-center gap-4 relative overflow-hidden"
                    >
                      {/* Colored strip left */}
                      <div className={`absolute top-0 left-0 bottom-0 w-1 ${isWin ? 'bg-emerald-500' : isLoss ? 'bg-red-500' : 'bg-zinc-500'}`} />

                      {/* 1. Result Badge (col-span-2) */}
                      ...
                      {/* 4. Time, Round, Date (col-span-3 text-right) */}
